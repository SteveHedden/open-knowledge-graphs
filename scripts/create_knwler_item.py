#!/usr/bin/env python3
"""Create a Wikidata item for Knwler, missing from the OKG catalog.

Confirmed via multiple Wikidata searches (label variants, GitHub repo URL,
maintainer "Orbifold") that no existing item covers this project.

Usage:
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_knwler_item.py
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_knwler_item.py --dry-run
"""
...
EDIT_SUMMARY = "Creating item for Knwler, missing from OKG catalog (via OKG catalog bot)"
PAUSE = 1.0

ITEM = {
    "label": "Knwler",
    "description": "Open-source Python tool that extracts structured knowledge graphs from unstructured documents using LLMs",
    "claims": [
        ("item",   P_INSTANCE_OF, Q_SEMANTIC_WEB_SOFTWARE),
        ("item",   P_LICENSE,     Q_MIT_LICENSE),
        ("item",   P_LANGUAGE,    Q_PYTHON),
        ("string", P_SOURCE_REPO, "https://github.com/Orbifold/knwler"),
        ("string", P_WEBSITE,     "https://knwler.com/"),
        ("string", P_VERSION,     "1.1.0"),
    ],
}
