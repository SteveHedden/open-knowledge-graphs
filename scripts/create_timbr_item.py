#!/usr/bin/env python3
"""Create a Wikidata item for Timbr, missing from the OKG catalog.

Timbr is commercial/proprietary (built by WP Semantix Ltd.) with no public
source repository or disclosed tech stack/version — unlike the other items
created this session, so this item only carries instance-of, license
(proprietary), and official website claims.

Confirmed via multiple Wikidata searches (label, domain string, company
name) that no existing item covers this project.

Usage:
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_timbr_item.py
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_timbr_item.py --dry-run
"""
...
Q_PROPRIETARY_LICENSE   = "Q3238057"  # same value used for Ontotext GraphDB's license in the existing catalog

EDIT_SUMMARY = "Creating item for Timbr, missing from OKG catalog (via OKG catalog bot)"
PAUSE = 1.0

ITEM = {
    "label": "Timbr",
    "description": "Commercial ontology-based semantic layer that virtualizes and unifies enterprise data sources into a queryable knowledge graph",
    "claims": [
        ("item",   P_INSTANCE_OF, Q_SEMANTIC_WEB_SOFTWARE),
        ("item",   P_LICENSE,     Q_PROPRIETARY_LICENSE),
        ("string", P_WEBSITE,     "https://timbr.ai/"),
    ],
}
