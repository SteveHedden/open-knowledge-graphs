#!/usr/bin/env python3
"""Create a Wikidata item for RDFGraphGen, missing from the OKG catalog.

Confirmed via multiple Wikidata search angles (label variants, exact GitHub
repo URL, full-text search) that no existing item covers this project.

RDFGraphGen generates synthetic RDF graphs from SHACL shape definitions —
inverting SHACL's usual validation role into a data-generation tool, for
benchmarking/testing/training use cases in the RDF/semantic web domain.

Usage:
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_rdfgraphgen_item.py
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_rdfgraphgen_item.py --dry-run
"""
...
Q_MIT_LICENSE           = "Q334661"
Q_PYTHON                = "Q28865"

EDIT_SUMMARY = "Creating item for RDFGraphGen, missing from OKG catalog (via OKG catalog bot)"
PAUSE = 1.0

ITEM = {
    "label": "RDFGraphGen",
    "description": "Python tool that generates synthetic RDF graphs satisfying given SHACL shape constraints, for benchmarking, testing, and training use cases",
    "claims": [
        ("item",   P_INSTANCE_OF, Q_SEMANTIC_WEB_SOFTWARE),
        ("item",   P_LICENSE,     Q_MIT_LICENSE),
        ("item",   P_LANGUAGE,    Q_PYTHON),
        ("string", P_SOURCE_REPO, "https://github.com/etnc/RDFGraphGen"),
        ("string", P_WEBSITE,     "https://github.com/etnc/RDFGraphGen"),
        ("string", P_VERSION,     "1.2"),
    ],
}
