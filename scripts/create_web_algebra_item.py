#!/usr/bin/env python3
"""Create a Wikidata item for AtomGraph/Web-Algebra, missing from the OKG catalog.

AtomGraph's other two projects already have Wikidata items and are in the OKG
catalog (Processor Q124653440, LinkedDataHub Q124653439) — this follows the
same claim shape as Processor (P31 semantic web software, P275 license,
P277 language, P1324 source repo, P856 website).

Usage:
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_web_algebra_item.py
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_web_algebra_item.py --dry-run
"""
...
# Value QIDs
Q_SEMANTIC_WEB_SOFTWARE = "Q124653107"  # same class used for Processor/LinkedDataHub
Q_APACHE_2              = "Q13785927"   # matches Processor's license claim
Q_PYTHON                = "Q28865"

EDIT_SUMMARY = "Creating item for AtomGraph/Web-Algebra, missing from OKG catalog (via OKG catalog bot)"
PAUSE = 1.0

ITEM = {
    "label": "Web-Algebra",
    "description": "Python framework and MCP tools that translate natural language into a JSON DSL for RDF/SPARQL Linked Data operations",
    "claims": [
        ("item",   P_INSTANCE_OF, Q_SEMANTIC_WEB_SOFTWARE),
        ("item",   P_LICENSE,     Q_APACHE_2),
        ("item",   P_LANGUAGE,    Q_PYTHON),
        ("string", P_SOURCE_REPO, "https://github.com/AtomGraph/Web-Algebra"),
        ("string", P_WEBSITE,     "https://pypi.org/project/web-algebra/"),
        ("string", P_VERSION,     "1.5.0"),
    ],
}
