#!/usr/bin/env python3
"""Create a Wikidata item for VocBench, missing from the OKG catalog.

VocBench is developed by the ART Research Group (University of Rome Tor
Vergata), currently funded under the EU's Digital Europe Programme. Source
is hosted on Bitbucket (not GitHub): bitbucket.org/art-uniroma2/vocbench3.
License confirmed directly from the repo's LICENSE file: 3-Clause BSD.

No programming-language claim: VocBench is a multi-component platform (an
Angular frontend over the Java-based Semantic Turkey engine), not a single-
language codebase like the other items added this session.

Usage:
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_vocbench_item.py
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_vocbench_item.py --dry-run
"""
...
EDIT_SUMMARY = "Creating item for VocBench, missing from OKG catalog (via OKG catalog bot)"
PAUSE = 1.0

ITEM = {
    "label": "VocBench",
    "description": "Web-based, multilingual, collaborative platform for managing OWL ontologies, SKOS(/XL) thesauri, Ontolex-lemon lexicons, and RDF datasets",
    "claims": [
        ("item",   P_INSTANCE_OF, Q_SEMANTIC_WEB_SOFTWARE),
        ("item",   P_LICENSE,     Q_BSD_3_CLAUSE),
        ("string", P_SOURCE_REPO, "https://bitbucket.org/art-uniroma2/vocbench3"),
        ("string", P_WEBSITE,     "https://vocbench.uniroma2.it/"),
        ("string", P_VERSION,     "15.1.2"),
    ],
}
