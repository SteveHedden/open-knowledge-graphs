#!/usr/bin/env python3
"""Create a Wikidata item for TrustGraph, missing from the OKG catalog.

Confirmed via multiple Wikidata search angles (label, domain, exact GitHub
repo URL, full-text search of all 22 "TrustGraph" hits, founder names) that
no existing item covers this project. The closest false-positive, Q138859511
("Trust Graph"), is an unrelated identity/fraud-detection product by Incode.

TrustGraph is an open-source AI-agent context/GraphRAG platform built on
OWL/SHACL/RDF 1.2, co-founded by Daniel Davis and Mark Adams.

Usage:
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_trustgraph_item.py
  WIKI_USER="Lemoncheddar@okg-updater" WIKI_PASS="botpassword" python3 scripts/create_trustgraph_item.py --dry-run
"""
...
EDIT_SUMMARY = "Creating item for TrustGraph, missing from OKG catalog (via OKG catalog bot)"
PAUSE = 1.0

ITEM = {
    "label": "TrustGraph",
    "description": "Open-source platform for grounding AI agents with explainable context via ontology-driven knowledge graphs (OWL, SHACL, RDF 1.2) combined with GraphRAG retrieval",
    "claims": [
        ("item",   P_INSTANCE_OF, Q_SEMANTIC_WEB_SOFTWARE),
        ("item",   P_LICENSE,     Q_APACHE_2),
        ("item",   P_LANGUAGE,    Q_PYTHON),
        ("string", P_SOURCE_REPO, "https://github.com/trustgraph-ai/trustgraph"),
        ("string", P_WEBSITE,     "https://trustgraph.ai/"),
        ("string", P_VERSION,     "2.6.14"),
    ],
}
